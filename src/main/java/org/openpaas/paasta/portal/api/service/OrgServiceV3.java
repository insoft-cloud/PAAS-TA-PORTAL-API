...
public class OrgServiceV3 extends Common {
    ...
    @Autowired
    private SpaceServiceV3 spaceServiceV3;
    ...
    public Map renameOrgForAdmin(Org org) {
        ...
        try {
            cloudFoundryClient().organizations().update(UpdateOrganizationRequest.builder().organizationId(org.getGuid().toString()).name(org.getNewOrgName()).quotaDefinitionId(org.getQuotaGuid()).build()).block();
    ...
    public Map deleteOrg(String orgId, boolean recursive, String token) throws Exception {
        ...
        try {
            ...
            final User user = userServiceV3.getUser(token);
    ...
    public Map getOrgDetail(String guid, String token) {
        ...
        try {
            ...
            SpaceResource spaces = listSpacesResponse.getResources().get(0);
            String spaceId=spaces.getMetadata().getId();

            Map<String, Collection<UserRole>> userRoles = getOrgUserRoles2(spaceId,guid, client);
    ...
    public Map<String, Collection<UserRole>> getOrgUserRoles2(String spaceId,String orgId, ReactorCloudFoundryClient reactorCloudFoundryClient) {
        Map<String, UserRole> userRoles = new HashMap<>();
        listAllOrgUsers(orgId, reactorCloudFoundryClient).stream().map(resource -> UserRole.builder().userId(resource.getMetadata().getId()).userEmail(resource.getEntity().getUsername()).modifiableRoles(true).build()).filter(ur -> null != ur).forEach(ur -> userRoles.put(ur.getUserId(), ur));
    ...
    private AssociateOrganizationManagerResponse associateOrgManager(String orgId, String userId, ReactorCloudFoundryClient reactorCloudFoundryClient) {
        spaceServiceV3.associateAllSpaceUserRolesByOrgId(orgId, userId, targetSpaceRole(OrgServiceV3.OrgRole.OrgManager), reactorCloudFoundryClient);
    ...
    private AssociateOrganizationBillingManagerResponse associateBillingManager(String orgId, String userId, ReactorCloudFoundryClient reactorCloudFoundryClient) {
        ...
        spaceServiceV3.associateAllSpaceUserRolesByOrgId(orgId, userId, targetSpaceRole(OrgServiceV3.OrgRole.BillingManager), reactorCloudFoundryClient);
    ...
    private AssociateOrganizationAuditorResponse associateOrgAuditor(String orgId, String userId, ReactorCloudFoundryClient reactorCloudFoundryClient) {
        spaceServiceV3.associateAllSpaceUserRolesByOrgId(orgId, userId, targetSpaceRole(OrgServiceV3.OrgRole.OrgAuditor), reactorCloudFoundryClient);
    ...
    public AbstractOrganizationResource associateOrgUserRole(String orgId, String userId, String role, String token) {
        try {
            ...
            final OrgServiceV3.OrgRole roleEnum;
            try {
                roleEnum = OrgServiceV3.OrgRole.valueOf(role);
    ...
    private Set<String> targetSpaceRole(OrgServiceV3.OrgRole orgRole) {
        final Set<String> targetSpaceRole = new HashSet<>();
        switch (orgRole) {
            case OrgManager:
            case ORGMANAGER:
                targetSpaceRole.add(OrgServiceV3.SpaceRole.SpaceManager.name());
            case OrgAuditor:
            case ORGAUDITOR:
                targetSpaceRole.add(OrgServiceV3.SpaceRole.SpaceDeveloper.name());
            case BillingManager:
            case BILLINGMANAGER:
                targetSpaceRole.add(OrgServiceV3.SpaceRole.SpaceAuditor.name());
                break;
            default:
    ...
    private void removeAllRoles(String orgId, String userId) {
        try {
            ...
            spaceServiceV3.removeAllSpaceUserRolesByOrgId(orgId, userId, targetSpaceRole(OrgServiceV3.OrgRole.OrgManager));
    ...
    public boolean associateOrgUserRole2(Map body) {
        try {
            ...
            String userId = userServiceV3.getUserIdByUsername(userEmail);
